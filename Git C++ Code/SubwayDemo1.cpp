...
void printHeader(int no=70,char c='-'){
    for(int i=0;i<no;i++){
        cout<<c;
    }
    cout<<endl;
...
void printStationTopics(){
    cout<<left<<setw(5)<<"No."<<setw(20)<<"Name"<<setw(15)<<"Platforms"<<setw(70)<<"Accessible for Disabled!"<<endl;
}

void printLineTopics(){
    cout<<left<<setw(5)<<"No."<<setw(20)<<"Name"<<setw(20)<<"Stations"<<endl;
}
...
public:
    ...
    void showDetails(int no){
        cout<<boolalpha;
        cout<<left<<setw(5)<<no+1<<setw(20)<<name<<setw(15)<<numberOfPlatforms<<setw(20)<<isAccesible<<endl;
    }
...
void removeStation(){
    ...
    cin>>index;
    for(int i=0;i<lines_count;i++){
        if(sub_lines[i].checkStation(sub_st[index])){
            sub_lines[i].removeStation(&sub_st[index]);
            break;
        }
    }
